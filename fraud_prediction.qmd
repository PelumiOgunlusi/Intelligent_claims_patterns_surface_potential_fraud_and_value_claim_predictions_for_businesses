...
```{python}
# SHAP interpretation for Random Forest model
import shap
```
...
```{python}
# Example: Force plot for a single prediction (updated for SHAP v0.20+)
shap.initjs()
...
import joblib
joblib.dump(rand_clf, 'random_forest_model.pkl')

# 2. Example: Load the model and make a prediction
# (In production, this would be in your API or app code)
model = joblib.load('random_forest_model.pkl')
sample = x_test.iloc[[0]]  # Example input
prediction = model.predict(sample)
print(f"Predicted fraud status: {prediction[0]}")

# 3. To deploy as an API, install FastAPI and Uvicorn:
# !pip install fastapi uvicorn joblib pandas

# 4. Example FastAPI app (save as app.py):
"""
